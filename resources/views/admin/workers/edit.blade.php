...
@section('title', 'تعديل بيانات العاملة')
        ...
        <form method="POST" action="{{ route('admin.workers.update', $worker) }}" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="nationality" value="{{ old('nationality', $worker->nationality) }}" required
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">العمر *</label>
                    <input type="number" name="age" value="{{ old('age', $worker->age) }}" min="18" max="60" required
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">الديانة</label>
                    <input type="text" name="religion" value="{{ old('religion', $worker->religion) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    ...
                    <select name="marital_status" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="عزباء" {{ old('marital_status', $worker->marital_status) == 'عزباء' ? 'selected' : '' }}>عزباء</option>
                        <option value="متزوجة" {{ old('marital_status', $worker->marital_status) == 'متزوجة' ? 'selected' : '' }}>متزوجة</option>
                        <option value="مطلقة" {{ old('marital_status', $worker->marital_status) == 'مطلقة' ? 'selected' : '' }}>مطلقة</option>
                        <option value="أرملة" {{ old('marital_status', $worker->marital_status) == 'أرملة' ? 'selected' : '' }}>أرملة</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">اللغات</label>
                    <input type="text" name="language" value="{{ old('language', $worker->language) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">الطول (سم)</label>
                    <input type="number" name="height" value="{{ old('height', $worker->height) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">الوزن (كغ)</label>
                    <input type="number" name="weight" value="{{ old('weight', $worker->weight) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">سنوات الخبرة</label>
                    <input type="number" name="experience_years" value="{{ old('experience_years', $worker->experience_years) }}" min="0"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">الراتب المتوقع (ريال)</label>
                    <input type="number" name="expected_salary" value="{{ old('expected_salary', $worker->expected_salary) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
            ...
            <div>
                ...
                <input type="text" name="skills"
                       value="{{ old('skills', $worker->skills ? implode(', ', $worker->skills) : '') }}"
                       placeholder="مثال: طبخ, تنظيف, رعاية أطفال"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">دول العمل السابقة (مفصولة بفاصلة)</label>
                <input type="text" name="previous_countries"
                       value="{{ old('previous_countries', $worker->previous_countries ? implode(', ', $worker->previous_countries) : '') }}"
                       placeholder="مثال: السعودية, الكويت"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('notes', $worker->notes) }}</textarea>
            </div>

            <div class="flex items-center gap-3">
                <input type="checkbox" name="is_featured" id="is_featured" value="1"
                       {{ old('is_featured', $worker->is_featured) ? 'checked' : '' }}
